...
def create_densenet(input_shape = (64,64,3),dense_truncated_layer='conv3_block2_concat'):
    ...
    dens_encoder = keras.Model(
        ...
        outputs=densnet.get_layer(dense_truncated_layer).output)#
    print(f'layer of TE is {count_layers(dens_encoder)}')
    #x = stn(inp,[64,64,100],kernel_size=(5,5),stage=1)
    x = dens_encoder(inp)
    x = Conv2D(
        kernel_size=(1,1),filters=300,padding='same',
        kernel_initializer='he_normal')(x)
    #x = stn(x,[100,100,100], stage=2)
    x = conv_block(x,kernel_size=(3,3),n_filters=300,strides=(1,1))
