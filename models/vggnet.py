...
def create_mobilenet(input_shape = (64,64,3)):
    ...
    mbilenet = VGG16(input_shape=input_shape,weights='imagenet',include_top=False)
    mbilenet.summary()
    dens_encoder = keras.Model(inputs=mbilenet.inputs,outputs=mbilenet.get_layer('block2_pool').output)
    x = dens_encoder(inp)
    #x = keras.layers.Conv2D(
        #kernel_size=(1,1),filters=128,padding='same',
    ...
    x = conv_block(x,kernel_size=(7,7),n_filters=128,strides=(2,2))
    #x = keras.layers.MaxPooling2D(pool_size=(2,2),strides=(2,2))(x)
    x = conv_block(x,kernel_size=(3,3),n_filters=128,strides=(2,2))
