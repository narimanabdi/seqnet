...
def create_vgg(input_shape = (64,64,3)):
    inp = keras.layers.Input(input_shape)
    vgg = VGG16(input_shape=input_shape,weights='imagenet',include_top=False)
    vgg_encoder = keras.Model(inputs=vgg.inputs,outputs=vgg.get_layer('block3_pool').output)
...
def create_model(input_shape = (64,64,3)):
    ...
    encoder = create_vgg(input_shape=input_shape)
