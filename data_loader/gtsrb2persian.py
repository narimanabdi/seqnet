###################################
# Generator for GTSRB --> Persian #
###################################
from .batchgenerator import PERSIAN_Generator,GTSRB_Generator

def get_generator(batch=128,dim=64):
    tr_gen = GTSRB_Generator(
        n_way=43,k_shot=1,batch=batch,data_type='all',
        target_size=(dim,dim),augmentation=True)
    te_gen = PERSIAN_Generator(
        n_way=43,k_shot=1,batch=batch,data_type='all',target_size=(dim,dim))
    return tr_gen,te_gen

def get_test_generator(batch=128,dim=64):
   return PERSIAN_Generator(
    n_way=43,k_shot=1,batch=batch,data_type='all',target_size=(dim,dim))